...
def append_to_kwargs(kwargs:Dict, key:str, value:str):
    ...
    string type.
    '''
    if isinstance(kwargs.setdefault(key, []), str):
...
def main():
    ...
    VSCODEDIR = path.join(WORKDIR, "vscode")
    PACKAGINGDIR = path.join(WORKDIR, "packaging")

    DARWIN_SELF_SIGN_SCRIPT = "macos-self-sign.command"

    # paths relative to the build.ninja
    VSCODEDIR_REL = path.relpath(VSCODEDIR, PACKAGINGDIR)
    WORKDIR_REL = path.relpath(WORKDIR, PACKAGINGDIR)

    # get product info (for obtaining product names)
    with open(path.join(VSCODEDIR, "product.json"), "r") as f:
    ...
    # output build ninja
    PACKAGINGDIR_REL = path.relpath(PACKAGINGDIR, BASEDIR)
    print(f"Creating build.ninja in '{PACKAGINGDIR_REL}' ...")
    os.makedirs(PACKAGINGDIR_REL, exist_ok=True)
    with open(path.join(PACKAGINGDIR_REL, "build.ninja"), "w") as f:
        wr = NinjaWriter(f)
        ...
        for reh in [True, False]:
            ...
            if not reh and PRIMARY_TARGET[0] == 'darwin':
                if args.signcertname:
                    ...
                    app_bundle_sign_target = path.join(app_bundle_path, "Contents/_CodeSignature")
                    wr.build(
                        app_bundle_sign_target, "darwinsign", inputs=[primary_package],
                        variables={
