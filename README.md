...
<p align="center">
  <img src="assets/icon.png" alt="Aether icon" height="128">
</p>

Aether is a fork of [Visual Studio Code OSS](https://github.com/microsoft/vscode) with my personal modifications, particularly tailored to significantly enhance the remote development experience. Additionally, it includes numerous quality-of-life improvements that cannot be implemented as an extension and are not approved by the upstream maintainers.

**NOTE:** CLI is now named `ae` instead of `code`.

## Changes compared to commercial Visual Studio Code

While Aether is based on [Visual Studio Code](https://code.visualstudio.com), it differs from the commercial version in the following ways:

- **No GitHub extensions.** GitHub's official extensions will not be included in Aether.
- **No Microsoft authentication support.** Microsoft's authentication provider is not included: you will not be able to sign in with a Microsoft account.
- **No Copilot chat support.** GitHub Copilot chat integration is not available.
- **Legacy libc Linux REH support.** The REH package includes builds for Linux distributions with legacy libc (e.g., Enterprise Linux 7, Ubuntu 18.04).

### VSDA

VSDA (IYKYK 🙂) is no longer supported in Aether. Microsoft's extensions that expects them will refuse to work. Consider these alternatives:

- **PyLance** → [basedpyright](https://marketplace.visualstudio.com/items?itemName=detachhead.basedpyright)
- **Microsoft C++ Extension** → [clangd](https://marketplace.visualstudio.com/items?itemName=llvm-vs-code-extensions.vscode-clangd)
...
- **Legacy libc Linux REH support.**  
  The REH package includes builds that support Linux distributions with legacy libc (e.g., Enterprise Linux 7, Ubuntu 18.04).
